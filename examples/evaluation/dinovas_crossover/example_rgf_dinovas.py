...
def run_dinova_example(n_steps, 
    ...
    for timestep in tqdm(range(NUM_TIMESTEPS)):
        ...
        for robot_id in range(NUM_ROBOTS):
            ...
            print(f"Robot {robot_id}: {planner.error(goal_pos=T_W_Objects[robot_id][:3, 3], q_current=robot_states[robot_id][0])}")
            if planner.error(goal_pos=T_W_Objects[robot_id][:3, 3], q_current=robot_states[robot_id][0]) <= stopping_tolerance:
