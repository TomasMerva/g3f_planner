...
def run_dinova_example(n_steps, dof, n_robots, env:Environment, nr_obst = 3, render=False, stopping_tolerance=0.05):
    RENDER = render
    NUM_ROBOTS = n_robots
    NUM_DOF = dof
    ...
    if NUM_OBST == 3:
        ...
        config_path = os.path.join(current_script_dir,'..', 'config/dinova_config_rgf_6obst.yaml')
    elif NUM_OBST == 9:
    ...
    for timestep in tqdm(range(NUM_TIMESTEPS)):
        ...
        for robot_id in range(NUM_ROBOTS):
            ...
            end_time = time.perf_counter() #todo: good place for timer
