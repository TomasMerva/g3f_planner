...
def run_kinova_example(n_steps=5000, render=True, dof=9, nr_robots=2):
    ...
    nr_waypoints = 10
    nr_inner_optim = 10
    nr_obst_multirobot = (nr_robots - 1) * 6
    collision_links = [
        "chassis_link",
        "arm_forearm_link",
    ...
    goals_final = [subgoal0]*nr_robots
    ...
    """"
       ...
       """
    deadlock_prevention = DeadlockPrevention(dof=[dof, dof], n_robots=nr_robots)
    ...
    for i in range(1, nr_waypoints):
        ...
        gomp_planner.add_collision_constraint(name= g_name, 
                                              ...
                                              r_link= 0.5, 
                                              r_obst=0.2)
        gomp_planner.param_dict[g_name]["num_param"] = ob['robot_0']['FullSensor']['obstacles'][nr_obst+nr_robots-1]['position']
        ...
        gomp_planner.add_collision_constraint(name= g_name, 
                                              ...
                                              r_link= 0.5, 
                                              r_obst=0.2)
        gomp_planner.param_dict[g_name]["num_param"] = ob['robot_0']['FullSensor']['obstacles'][nr_obst+nr_robots]['position']
    ...
    for w in range(n_steps):
        ...
        xee_dinovas = [FK_W["arm_tool_frame"].get_matrix().numpy()[i_robot,:3,3] for i_robot in range(nr_robots)]
        ...
        x_obsts_env = []
        radius_obsts_env = []
        qdot_rollout_avg = {"robot_0": [], "robot_1": []}
        ...
        for i_robot in range(nr_robots):
            ...
            if w % 100 == 0:
                ...
                q_rollout = rollouts_planner.compute_rollout(timesteps=nr_rollout_timesteps,
                                                             ...
                                                             tolerance=0.15)
                qdot_rollout_avg["robot_"+str(i_robot)] = rollouts_planner.get_rollout_velocity_avg()
                ...
                if q_fabrics_initial_guess is not None:
                    q_prev_solutions["robot_"+str(i_robot)] = copy.deepcopy(q_fabrics_initial_guess)
                ...
                if i_robot == 1: # you need the info of both robots for the deadlock detection:
                    deadlock_prevention.deadlock_checking(x_robots=xee_dinovas,
                                                          goals_final=goals_final,
                                                          time_step=w,
                                                          avg_sum=sum(qdot_rollout_avg.values())/nr_robots)
            #
        ...
        # actions robot 0:
        for j_robot in range(nr_robots):
            goal_robots, goal_weights = deadlock_prevention.deadlock_adapt_goals_weights(x_robots=xee_dinovas,
                                                                                         goal_robots=[arguments_dicts["robot_"+str(j_robot)]["x_goal_0"] for j_robot in range(nr_robots)],
                                                                                         goal_weights=[arguments_dicts["robot_"+str(j_robot)]["weight_goal_0"] for j_robot in range(nr_robots)])
            arguments_dicts["robot_" + str(j_robot)]["x_goal_0"] = goal_robots[j_robot]
            arguments_dicts["robot_" + str(j_robot)]["weight_goal_0"] = goal_weights[j_robot]
        print("arguments_dicts[robot_0]", arguments_dicts["robot_0"]["weight_goal_0"])
        ...
        # actions robot 1:
        if nr_robots>1:
            print("arguments_dicts[robot_1]", arguments_dicts["robot_1"]["weight_goal_0"])
