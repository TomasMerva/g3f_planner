...
from g3f_planner import Solver
from g3f_planner import RolloutFabrics
...
class G3F_Planner():
    def __init__(self, fk_args, config_file_path) -> None:
        ...
        self._roll_obj_grasp =  np.deg2rad(self._CONFIG["g3f"]["initial_grasp_roll_deg"])
        ...
        self.num_waypoints = self._CONFIG["g3f"]["n_waypoints"]
        self.num_obstacles = self._CONFIG["problem"]["environment"]["number_spheres"]["static"]
        self.num_optim_steps = self._CONFIG["g3f"]["n_optim_steps"]
    ...
    def establish_rollouts(self) -> None:
        ...
        _fabrics_lib = os.path.normpath(os.path.join(_current_script_dir,  '../build/', self._CONFIG["g3f"]["fabrics_lib"]))
        ...
        self._rollouts_planner = RolloutFabrics(fk_dict=self._fk_args,
                                                ...
                                                dt=self._CONFIG["g3f"]["rollout_dt"],
                                                alpha_filter=self._CONFIG["g3f"]["alpha_filter"])
    ...
    def establish_planner(self):
        gomp_args = dict(
            ...
            num_waypoints = self._CONFIG["g3f"]["n_waypoints"],
            num_dim = self._fk_args["num_dofs"],
            joint_limits = self._CONFIG["problem"]["joint_limits"]
        )

        self._gomp_planner = Solver(gomp_args)
        ...
        self._gomp_planner.setup_problem(x0=np.ones((self.num_waypoints, self.num_dofs)),
                                         verbose = self._CONFIG["g3f"]["verbose"]
                                         )
    ...
    def _establish_collision_constraint(self):
        ...
        for link_idx, link_name in enumerate(self._CONFIG["g3f"]["collision_links"]):
            self.collision_links[link_name] = self._CONFIG["g3f"]["collision_link_radius"][link_idx]
    ...
    def _compute_initial_guesses(self):
        ...
        q_coll_rollout = self._rollouts_planner.compute_rollout(
                                                timesteps=self._CONFIG["g3f"]["rollout_timesteps"],
                                                arg_dict=self._rollouts_args_dict,
                                                tolerance=self._CONFIG["g3f"]["rollout_tolerance"]
                                                )
