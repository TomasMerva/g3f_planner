...
def set_planner(robot_urdf_path, config_dict, degrees_of_freedom: int = 9, nr_obst_multirobot=0):
    ...
    config_problem = config_dict['problem']
    config_problem['environment']['number_spheres']['static'] = config_problem['environment']['number_spheres']['static'] + nr_obst_multirobot
...
def run_kinova_example(n_steps=5000, render=True, dof=9, nr_robots=2):
    ...
    planner_dinova = set_planner(robot_urdf_path = env.ROBOT_URDF_FILE,
                                 ...
                                 nr_obst_multirobot = nr_obst_multirobot)
    ...
    for w in range(n_steps):
        ...
        for col_link in collision_links:
            obst_dinovas["robot_0"].append( FK_W[col_link].get_matrix().numpy()[0,:3,3] )
            obst_dinovas["robot_1"].append( FK_W[col_link].get_matrix().numpy()[1,:3,3] )

        # compute arguments for the fabrics action
        x_obsts_env = [ob["robot_0"]['FullSensor']['obstacles'][nr_obst+nr_robots-1]['position'],
                       ob["robot_0"]['FullSensor']['obstacles'][nr_obst + nr_robots]['position']]
        radius_obsts_env = [ob["robot_0"]['FullSensor']['obstacles'][nr_obst + nr_robots-1]['size'],
                            ob["robot_0"]['FullSensor']['obstacles'][nr_obst + nr_robots]['size']]
...
if __name__ == "__main__":
    dof = 11
    nr_robots = 2
    res = run_kinova_example(n_steps=5000, dof=dof, nr_robots=2)